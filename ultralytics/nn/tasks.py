...
from ultralytics.nn.modules.fasternet import C3k2_Faster, C3k_Faster, DetectFaster, FasterNetBlock  # FasterNet
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, FasterNetBlock):
                    m.fuse_bn()
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in {C3k2, DSC3k2, C3k2_Faster}:  # for P/U sizes
                legacy = False
                if scale in "lx":
                    args[3] = True
            if m is A2C2f: 
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, DetectFaster}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, DetectFaster}:
                m.legacy = legacy
